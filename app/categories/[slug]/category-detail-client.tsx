...
/**
 * The interactive half of a category page.
 *
 * `initialCategory` / `initialProducts` are fetched by the server component in
 * page.tsx, so the first page of product cards — names, prices, links — is in
 * the served HTML instead of a skeleton. Filtering, sorting and paging past the
 * first page stay here on the client, which is where they belong.
 */
/**
 * How many products the server component pre-renders.
 *
 * Exported so page.tsx fetches exactly this many — the client uses it to
 * decide whether more pages remain, and the two drifting apart would either
 * truncate a category or fire a pointless extra request.
 */
...
export default function CategoryDetailPage({
  ...
  // Not "loading" when the server already supplied the first page — that would
  // paint a skeleton over content the HTML already has.
  ...
  // Prevent sidebar scroll from leaking to the page (non-passive wheel handler).
  ...
  // Sidebar reference data — categories to jump between. Skipped when the
  // server already sent them.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // The server rendered the first page already. Don't wipe it and refetch —
    // just continue collecting the remaining pages in the background, so large
    // categories still fill in without the visitor seeing a flash of empty.
    if (initialCategory) {
      let rows = initialProducts
      const continueLoading = async () => {
        let nextPage = 2
        // The server fetch asks for a full page; if it came back short there
        // is nothing more to collect.
        let hasNext = initialProducts.length >= SERVER_PAGE_SIZE
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-[260px] shrink-0">
            <div ref={sidebarRef} className="sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto overscroll-contain p-6 bg-card rounded-3xl border border-border shadow-sm">
