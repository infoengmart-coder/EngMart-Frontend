...
import { ConfirmDialog, useScrollLock } from '@/components/confirm-dialog'
...
export default function AddressesPage() {
  ...
  const confirmDelete = () => {
    if (pendingDeleteId) deleteAddress(pendingDeleteId)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Empty state — a fresh account has no saved addresses yet */}
      {addresses.length === 0 && (
        <div className="panel-card flex flex-col items-center justify-center py-14 text-center">
          <div className="w-14 h-14 rounded-full bg-secondary flex items-center justify-center mb-4">
            <MapPin className="w-7 h-7 text-muted-foreground" />
          </div>
          <h3 className="text-sm font-bold text-foreground mb-1">No saved addresses yet</h3>
          <p className="text-xs text-muted-foreground max-w-xs mb-4">
            Save a delivery address once and reuse it at every checkout.
          </p>
          <button
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
          <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl animate-[fadeIn_0.2s_ease-out] overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {formError && (
                <p className="rounded-lg border border-destructive/30 bg-destructive/5 text-destructive text-xs p-2.5">
                  {formError}
                </p>
              )}
