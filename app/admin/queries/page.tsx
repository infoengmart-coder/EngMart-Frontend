  ...
  return (
    <div className="p-5 md:p-8 flex flex-col h-[calc(100vh-64px)] md:h-screen min-w-0">
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-5 min-h-0 pb-5 overflow-hidden">
        {/* Inbox List */}
        <div className="lg:w-1/3 bg-card rounded-xl border border-slate-300 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden shrink-0">
          <div className="p-3 border-b border-slate-300 dark:border-slate-700 bg-card flex gap-1.5 shrink-0">
            <button
              onClick={() => setTabFilter("All")}
              className={`px-2 py-1.5 text-[10px] font-bold rounded-md flex-1 text-center transition-all ${
                tabFilter === "All" ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-muted-foreground hover:text-foreground border border-slate-200 dark:border-slate-800"
              }`}
            >
              All
            </button>
            <button
              ...
              className={`px-2 py-1.5 text-[10px] font-bold rounded-md flex-1 text-center transition-all ${
                tabFilter === "new" ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-muted-foreground hover:text-foreground border border-slate-200 dark:border-slate-800"
              }`}
            >
              New ({getCount("new")})
            </button>
            <button
              ...
              className={`px-2 py-1.5 text-[10px] font-bold rounded-md flex-1 text-center transition-all ${
                tabFilter === "replied" ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-muted-foreground hover:text-foreground border border-slate-200 dark:border-slate-800"
              }`}
            >
              Replied
            </button>
            <button
              ...
              className={`px-2 py-1.5 text-[10px] font-bold rounded-md flex-1 text-center transition-all ${
                tabFilter === "closed" ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-muted-foreground hover:text-foreground border border-slate-200 dark:border-slate-800"
              }`}
            >
              Closed
            </button>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-300 dark:divide-slate-700">
        ...
        {activeQuery ? (
          <div className="lg:w-2/3 bg-card rounded-xl border border-slate-300 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden min-w-0">
            <div className="p-5 border-b border-slate-300 dark:border-slate-700 bg-card flex justify-between items-start gap-4 shrink-0">
              <div>
              ...
              <div className="flex gap-1.5 shrink-0">
                <button 
                  ...
                  className="p-2 rounded-lg border border-slate-300 dark:border-slate-700 text-muted-foreground hover:text-[var(--color-success)] hover:border-[var(--color-success)] transition-colors" 
                  title="Close Query"
                >
                ...
                <button 
                  ...
                  className="p-2 rounded-lg border border-slate-300 dark:border-slate-700 text-destructive hover:bg-destructive/10 hover:border-destructive transition-colors" 
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-background/30 space-y-4">
              <div className="bg-card p-5 rounded-lg text-xs text-foreground leading-relaxed whitespace-pre-wrap border border-slate-300 dark:border-slate-700 shadow-sm">
                {activeQuery.message}
              </div>
            </div>

            {/* Reply Editor */}
            <form onSubmit={handleSendReply} className="p-4 border-t border-slate-300 dark:border-slate-700 bg-card shrink-0 space-y-3">
              <textarea
                value={replyText}
                onChange={e => setReplyText(e.target.value)}
                className="w-full px-3 py-2 text-xs border border-slate-300 dark:border-slate-700 rounded-lg bg-card text-foreground outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary min-h-[90px] resize-none"
              ...
              <div className="flex justify-between items-center">
                <div className="flex gap-2">
                  <button 
                    ...
                    className="text-[11px] font-semibold py-1.5 px-3 rounded-lg border border-slate-300 dark:border-slate-700 text-foreground hover:bg-secondary transition-colors"
                  >
                    Mark Replied
                  </button>
                  <button 
          ...
          <div className="lg:w-2/3 bg-card rounded-xl border border-slate-300 dark:border-slate-700 shadow-sm flex items-center justify-center p-10 text-center">
