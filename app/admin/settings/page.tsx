...
export default function SettingsPage() {
  ...
  // Store information — seeded from the live settings context, then editable
  const { settings, refresh } = useSiteSettings();
  const [storeName, setStoreName] = useState(settings.name);
  const [storeTagline, setStoreTagline] = useState(settings.tagline);
  ...
  const [storeHours, setStoreHours] = useState(settings.hours);
  const [storeWhatsapp, setStoreWhatsapp] = useState(settings.whatsapp);

  // Payment method toggles
  const [codActive, setCodActive] = useState(settings.cod_enabled);
  const [bankActive, setBankActive] = useState(settings.bank_transfer_enabled);
  const [mobileActive, setMobileActive] = useState(settings.mobile_wallet_enabled);

  // New-customer welcome discount. Kept as a string so the number input can be
  // cleared while typing without snapping back to 0.
  const [welcomeActive, setWelcomeActive] = useState(settings.welcome_discount_enabled);
  const [welcomePercent, setWelcomePercent] = useState(String(settings.welcome_discount_percent ?? 5));

  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState("");

  // The context hydrates asynchronously — re-seed the form once it arrives.
  useEffect(() => {
    ...
    setStoreWhatsapp(settings.whatsapp);
    setCodActive(settings.cod_enabled);
    setBankActive(settings.bank_transfer_enabled);
    setMobileActive(settings.mobile_wallet_enabled);
    setWelcomeActive(settings.welcome_discount_enabled);
    setWelcomePercent(String(settings.welcome_discount_percent ?? 5));
  }, [settings]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      await updateSiteSettings({
        ...
        cod_enabled: codActive,
        bank_transfer_enabled: bankActive,
        mobile_wallet_enabled: mobileActive,
        welcome_discount_enabled: welcomeActive,
        // Clamp before saving: a negative or >100 percentage would produce a
        // nonsensical order total server-side.
        welcome_discount_percent: String(
          Math.min(100, Math.max(0, Number(welcomePercent) || 0)),
        ),
      });
      await refresh(); // push the new values into the live storefront context
    ...
    <div className="p-5 md:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          {saveMsg && (
            <span className={`text-xs font-semibold ${saveMsg.includes("saved") ? "text-emerald-600" : "text-destructive"}`}>
              {saveMsg}
            </span>
          )}
          <button onClick={handleSaveChanges} disabled={saving} className="btn-primary text-xs flex items-center gap-1.5 disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm p-6 space-y-4">
            <h2 className="text-sm font-bold text-foreground flex items-center gap-2 border-b border-border pb-3">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Welcome Discount
            </h2>

            <div className="flex items-center justify-between">
              <div className="pr-3">
                <h4 className="text-xs font-bold text-foreground">First-order discount</h4>
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  Shows the sign-up popup to new visitors and applies the discount
                  automatically to a customer&apos;s first order.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setWelcomeActive(!welcomeActive)}
                aria-pressed={welcomeActive}
                className="text-primary hover:text-primary/80 shrink-0"
              >
                {welcomeActive ? <ToggleRight className="w-8 h-8" /> : <ToggleLeft className="w-8 h-8 text-muted-foreground" />}
              </button>
            </div>

            <div>
              <label
                htmlFor="welcome-percent"
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setWelcomePercent(e.target.value)}
                  disabled={!welcomeActive}
                  className="input-base border border-border disabled:opacity-50"
                />
                <span className="text-sm font-bold text-muted-foreground">%</span>
              </div>
              <p className="text-[10px] text-muted-foreground mt-1.5">
                The storefront popup, the cart and the checkout all read this
                number, and the server applies the same figure to the order.
              </p>
            </div>
          </div>
