...
import {
  Activity, Briefcase, Building, Compass, Cpu, Gauge, Globe, Handshake,
  Headphones, HelpCircle, Layers, Plug, ShieldAlert, Sliders, Target,
  TrendingUp, Wrench, Zap, type LucideIcon,
} from 'lucide-react'

// Icons referenced by name from TIMELINE/TEAM/MISSION_CARDS below and BRANDS data.
const ICONS: Record<string, LucideIcon> = {
  Activity, Briefcase, Building, Compass, Cpu, Gauge, Globe, Handshake,
  Headphones, Layers, Plug, ShieldAlert, Sliders, Target, TrendingUp, Wrench, Zap,
}
...
export default function AboutPage() {
  return (
    <div className="min-h-screen bg-background transition-colors">
      ...
      {/* Page hero */}
      <section className="relative pt-14 pb-20 bg-card border-b border-border transition-colors overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }}>
              ...
              <h2 className="text-3xl sm:text-4xl font-black text-foreground mb-6 tracking-tight leading-tight">
            ...
            <motion.div initial={{ opacity: 0, x: 15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {MISSION_CARDS.map((card, idx) => (
                  <motion.div
                    key={idx}
                    className="glow-card block overflow-hidden bg-card border border-border p-6 group hover:border-primary/20 transition-colors duration-300"
                    initial={{ opacity: 0, y: 15 }}
                  ...
                  >
                    ...
                    <h3 className="text-sm font-bold text-foreground mb-2 group-hover:text-primary transition-colors">{card.title}</h3>
                    <p className="text-xs text-muted-foreground leading-relaxed font-semibold">{card.text}</p>
                  </motion.div>
                ))}
              </div>
            </motion.div>
      ...
      <section className="py-20 sm:py-24 transition-colors">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-foreground tracking-tight leading-none">
          ...
          <div className="relative">
            ...
            <div className="space-y-8">
              {TIMELINE.map((item, idx) => (
                ...
                >
                  <div
                    className="absolute left-7 top-4 w-8 h-8 rounded-full flex items-center justify-center text-primary bg-primary/10 -translate-x-1/2 border-2 border-card shadow shadow-primary/10"
                  >
                    {renderLucideIcon(item.icon, "w-4 h-4")}
                  </div>
                  <div className="glow-card block overflow-hidden bg-card border border-border p-6 flex-1">
      ...
      {/* Brands */}
      <section className="py-20 sm:py-24 bg-card border-y border-border transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            {BRANDS.map((brand, idx) => (
              <motion.div
                key={brand.slug}
                className="glow-card block overflow-hidden bg-card border border-border p-6 flex flex-col items-center text-center group hover:border-primary/20 transition-colors duration-300"
                initial={{ opacity: 0, y: 10 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: idx * 0.05 }}
              >
                <Link href={`/brands/${brand.slug}`} className="flex flex-col items-center">
                  <div
                    className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4 transition-transform duration-300 group-hover:scale-105 shadow-inner"
                    style={{
                  ...
                  <h3 className="font-bold text-foreground mb-1 group-hover:text-primary transition-colors">{brand.name}</h3>
                  <div className="flex items-center gap-1.5 mb-1 justify-center">
                    <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{brand.country}</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground font-semibold">via {brand.supplier}</p>
                </Link>
              </motion.div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {TEAM.map((member, idx) => (
              <div
                key={idx}
                className="glow-card block overflow-hidden bg-card border border-border p-6 text-center group hover:border-primary/20 transition-colors duration-300"
              >
                <div
                  className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-transform duration-300 group-hover:scale-105"
                  style={{
                ...
                <p className="text-xs text-muted-foreground leading-relaxed font-semibold">{member.desc}</p>
              </div>
            ))}
          </div>
        </div>
