        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {brands.map((brand, idx) => {
              // An admin-uploaded logo wins; otherwise use the bundled wordmark
              // set. None of the 72 brand rows carry an uploaded logo yet, so
              // without this fallback every card rendered a two-letter monogram.
              const logoSrc = (brand.logo ? mediaUrl(brand.logo) : null)
                || brandLogo(brand.name, brand.slug)
              return (
              <motion.div
                key={brand.slug}
