...
import { sortVariantsByRating } from '@/lib/catalog-server'
 ...
 *
 * `initialProduct` is fetched by the server component in page.tsx and passed
 * in, so this renders the REAL product during the server pass instead of a
 * skeleton — which is what puts the name, price, variants and specs into the
 * HTML crawlers receive. See lib/catalog-server.ts for the full reasoning.
 *
 * It still knows how to fetch for itself: if the server fetch failed (the API
 * sleeps on the free tier), the effect below recovers on the client rather
 * than leaving the customer looking at an error.
 */
export default function ProductDetailPage({
  ...
  slug: string
  initialProduct?: ApiProductDetail | null
}) {
  const { settings: SITE } = useSiteSettings()
  const { add, isInCart, getItemKey } = useCart()
  const { requireAuth } = useAuthGate()

  const [product, setProduct] = useState<ApiProductDetail | null>(initialProduct)
  // Only "loading" when the server gave us nothing — otherwise the first paint
  // would flash a skeleton over content the HTML already contains.
  ...
  useEffect(() => {
    // The server already fetched this product — refetching on mount would
    // throw away the SSR data and cost every visitor an extra round trip.
    ...
    async function load() {
      ...
      try {
        ...
        if (isMounted) {
          // Same ordering the server applies, so a client recovery cannot
          // disagree with an SSR pass that did succeed.
          if (data.variants && data.variants.length > 1) {
            data.variants = sortVariantsByRating(data.variants)
          }
          setProduct(data)
  ...
  // Brand-wide discount, straight off the embedded brand payload — the backend
  // already zeroes it while a campaign is paused.
  const discountPercent = brandDiscountPercent(product.brand)
  /** A variant's price after the brand discount, for the picker labels. */
  const netPrice = (value: string | number | null | undefined) =>
    discountedUnitPrice(Number(value) || 0, discountPercent)
  ...
  const doAddToCart = () => {
    ...
    add({
      ...
      // percentage, so storing the reduced figure would discount it twice.
      unitPrice: isPOR ? 0 : unitPrice,
      isPriceOnRequest: isPOR,
      discountPercent: isPOR ? 0 : discountPercent,
    }, qty)
  }
          ...
          <div>
            ...
            <div className="p-5 rounded-xl bg-card border border-border mb-6 shadow-sm">
              <div className="flex items-center justify-between gap-2 mb-1">
                ...
                {discountPercent > 0 && (
                  <span className="text-[10px] font-black uppercase tracking-wide text-white bg-gradient-to-r from-rose-600 to-red-500 px-2 py-0.5 rounded">
                    {discountPercent}% OFF{product.brand?.discount_label ? ` · ${product.brand.discount_label}` : ''}
                  </span>
                )}
              </div>
              {currentPrice !== null ? (
                <div className="flex items-baseline gap-2 flex-wrap">
                  <span className="text-3xl font-black text-foreground">
                    {formatPrice(discountPercent > 0 ? netPrice(currentPrice) : currentPrice)}
                  </span>
                  {discountPercent > 0 && (
                ...
                <div className="flex items-baseline gap-2 flex-wrap">
                  <span className="text-2xl font-black text-foreground">
                    {discountPercent > 0
                      ? formatPriceRange({
                          min: netPrice(product.price_range.min),
                          max: netPrice(product.price_range.max),
                        })
              ...
              {/* Only meaningful for a priced variant — there is nothing to take
                  a percentage off a price-on-request item. */}
              {discountPercent > 0 && currentPrice !== null && (
                <p className="mt-2 text-[11px] font-bold text-rose-600 dark:text-rose-400">
                  You save {formatPrice(currentPrice - netPrice(currentPrice))} per unit on this brand offer.
                </p>
              )}
            </div>
                ...
                >
                  {product.variants.map(v => (
                    <option key={v.id} value={v.id}>
                      {v.description || v.cat_no} — {v.price ? formatPrice(netPrice(v.price)) : 'Price on Request'}
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  product.variants.map(v => (
                    ...
                    >
                      ...
                      <span className="font-bold text-primary text-right">
                        {v.price ? formatPrice(netPrice(v.price)) : 'Quote'}
