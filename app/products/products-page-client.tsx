  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-1.5 max-h-56 overflow-y-auto overscroll-contain custom-scrollbar pr-1">
          <button
...
function ProductsPageContent({
  ...
  // The navbar's "View all results for …" link and every external/shared link
  // lands here as /products?q=<term>. Seeding straight from the URL (rather
  // than from '' plus an effect) means the very first fetch already carries the
  // term — previously the parameter was read by nobody at all, so arriving
  // from the header search showed the unfiltered catalogue.
  ...
  // Fetch categories and brands once — unless the server already did.
  useEffect(() => {
    if (initialCategories.length && initialBrands.length) return
    async function loadMeta() {
      try {
        ...
        // suppliers (SWAS, AT Electricals, JC) rather than manufacturer
        // marques. Their products stay in the catalogue and remain fully
        // searchable — only the name is kept out of this filter list.
        setBrands((brandData || []).filter(b => b.show_in_filters !== false))
      } catch (err) {
        console.error('Failed to load categories/brands', err)
      }
    }
    loadMeta()
  }, [initialCategories.length, initialBrands.length])
  ...
  /**
   * True until the first client-side fetch has been skipped.
   *
   * The server component renders the unfiltered first page, so refetching it
   * on mount would discard that HTML and cost every visitor a round trip
   * before they see anything. Any filter change afterwards re-runs normally,
   * because `fetchProductsList` changes identity when its inputs do.
   */
  const skipFirstFetch = useRef(initialProducts.length > 0 && !urlQuery)
  ...
  // Re-sync when ?q= changes while we are already mounted. Searching again
  // from the header does a client-side navigation to the same route, which
  // updates the URL without remounting this component, so without this the
  // results would keep showing the previous term.
  useEffect(() => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    // Mirror the term into the URL so results are shareable, survive a reload,
    // and land correctly in the browser's back/forward history.
    router.replace(term ? `/products?q=${encodeURIComponent(term)}` : '/products',
      { scroll: false })
  }
  ...
  const clearFilters = () => {
    ...
    setPage(1)
    if (urlQuery) router.replace('/products', { scroll: false })
  }
  ...
  // Prevent sidebar wheel events from propagating to the page.
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Always stop propagation; only prevent default at boundaries (or when not scrollable)
  ...
  return (
    <div className="min-h-screen bg-secondary/30 flex flex-col">
      ...
      <div className="bg-card border-b border-border py-3 z-30 sticky top-20 shadow-sm">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Brand wall, directly under the catalog search box. Trimmed heading
            here — the page already has its own H1, so a second large title
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-[260px] shrink-0">
            <div ref={sidebarRef} className="sticky top-36 max-h-[calc(100vh-11rem)] overflow-y-auto overscroll-contain custom-scrollbar p-5 bg-card rounded-lg border border-border shadow-sm">
          ...
          {/* Main Content */}
          <main ref={mainRef} className="flex-1 min-w-0 scroll-mt-36">
            {/* Toolbar */}
 ...
 * the boundary to be explicit. The fallback mirrors the page's own loading
 * chrome so there is no layout jump on first paint.
 */
export default function ProductsPage(props: SeedData) {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-secondary/30 flex flex-col">
