...
/**
 * Resolve a relative media path from the backend into a full URL.
 * e.g. "/media/products/img.jpg" → "http://localhost:8000/media/products/img.jpg"
 * Already-absolute URLs (http/https) or blobs/placeholders are returned unchanged.
 */
export function mediaUrl(path: string | null | undefined): string {
  if (!path) return '/product-placeholder.svg'
  if (path.startsWith('http://') || path.startsWith('https://') || path.startsWith('//') || path.startsWith('data:') || path.startsWith('blob:')) {
    return path
  }
...
function getAuthToken(): string | null {
  try {
    ...
    const raw = sessionStorage.getItem('engmart_tokens') || localStorage.getItem('engmart_tokens')
    return raw ? JSON.parse(raw).access : null
  } catch { return null }
}

async function authFetch<T>(endpoint: string, options: RequestInit & { _isRetry?: boolean } = {}): Promise<T> {
  const token = getAuthToken()
  ...
  // Handle 401 token expiry by trying refresh token once
    ...
    try {
      ...
      if (tokens?.refresh) {
        ...
        if (refreshRes.ok) {
          ...
          if (sessionStorage.getItem('engmart_tokens')) {
            sessionStorage.setItem('engmart_tokens', JSON.stringify(newTokens))
          } else {
            localStorage.setItem('engmart_tokens', JSON.stringify(newTokens))
          }
          // Retry the original request with the fresh token
          return authFetch<T>(endpoint, { ...options, _isRetry: true })
        }
      }
    } catch {}
  }
...
export type SiteSettingsData = {
  ...
  /** First-order welcome offer, editable from the admin dashboard. */
  welcome_discount_enabled: boolean
  welcome_discount_percent: string
  /** GST + cash-on-delivery charges, set in Admin -> Wholesale. */
  gst_enabled: boolean
  gst_percent: string
  cod_fee: string
}
...
/** What the signed-in customer's first-order welcome discount is worth. */
...
/**
 * Ask the server whether this customer's next order carries the welcome
 * discount, and what it is worth for `subtotal`.
 *
 * The cart and checkout only DISPLAY this. The same figure is recomputed
 * server-side when the order is placed, so the number shown here and the number
 * charged cannot diverge.
 */
export async function getWelcomeDiscount(subtotal: number): Promise<WelcomeDiscountInfo> {
  const query = subtotal > 0 ? `?subtotal=${encodeURIComponent(String(subtotal))}` : ''
  return authFetch<WelcomeDiscountInfo>(`/account/welcome-discount/${query}`)
}
